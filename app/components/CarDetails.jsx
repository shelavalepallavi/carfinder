...
const CarDetails = ({car}) => {
  ...
  return (
    <div className={`w-full min-h-screen px-10 pt-12 flex flex-col md:flex-row gap-12 ${theme === 'dark'? 'bg-gray-800 text-white':''}`}>
      <img src={car.image} alt={`${car.make} ${car.model}`} className='w-full md:w-1/2 h-2/3 aspect-video object-cover rounded-md' />
  ...
  <span>
    Crafted to complement your lifestyle — wherever the road takes you.
          ...
          <div className='flex gap-4'>
            ...
            <button className={`px-4 py-2 rounded-md font-semibold text-indigo-800 border border-indigo-800 hover:bg-indigo-700 hover:text-white cursor-pointer ${theme === 'dark'? 'bg-indigo-700 text-white':''}`}>Buy Now</button>
          </div>
      </div>
    </div>
  )
